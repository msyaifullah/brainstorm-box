...
export default function FlightDemoPage() {
  ...
  // Add state for multi-city segments
  const [multiCitySegments, setMultiCitySegments] = useState([
    { id: "1", from: "Jakarta, Indonesia (JKT)", to: "Singapore, Singapore (SIN)" },
    { id: "2", from: "Singapore, Singapore (SIN)", to: "Bangkok, Thailand (BKK)" },
  ]);
  ...
  // Search button click handler for one way
    ...
    <div className="demo-container">
      ...
      <div className="search-wrapper">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', justifyContent: 'flex-end' }}>
          ...
          <button
            ...
            onClick={
              tripType === "multi-city" ? () => handleMultiCitySearch(multiCitySegmentsWithDates) : handleRoundTripSearch
            }
            disabled={isLoading}
            style={{ width: '25%' }}
          >
      ...
      <div className="search-wrapper">
        <FlightSearch
          ...
          onSwap={handleSwap}
        />
        <button className="search-button" onClick={handleOneWaySearch} disabled={isLoading}>
          {isLoading ? "Searching..." : "Search Flights"}
        </button>
      </div>
      ...
      <div className="search-wrapper">
        <h3>Multi-City tanpa Tanggal Berangkat</h3>
        <FlightSearch isMultiCity={true} segments={multiCitySegments} onSegmentsChange={handleSegmentsChange} />
        <button className="search-button" onClick={() => handleMultiCitySearch(multiCitySegments)} disabled={isLoading}>
          {isLoading ? "Searching..." : "Search Flights"}
        </button>
      </div>

      {/* Multi-city with departure dates */}
      <div className="search-wrapper">
        <h3>Multi-City dengan Tanggal Berangkat</h3>
        <FlightSearch
          isMultiCity={true}
          segments={multiCitySegmentsWithDates}
          onSegmentsChange={handleMultiCitySegmentsChange}
        ...
        <button
          className="search-button"
          onClick={() => handleMultiCitySearch(multiCitySegmentsWithDates)}
          disabled={isLoading}
        >
          {isLoading ? "Searching..." : "Search Flights"}
        </button>
